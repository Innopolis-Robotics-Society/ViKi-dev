...
def test_manager_start_stop():
    ...
    with patch.object(CameraManager, '_make_backend', return_value=MockBackend("mock_dev")):
...
def test_manager_nearest_frame():
    ...
    with patch.object(CameraManager, '_make_backend', return_value=mock_backend):
        ...
        f1 = Frame(color=np.zeros((1,1,3), dtype=np.uint8), depth=np.zeros((1,1), dtype=np.uint16), timestamp_us=1000, device_id="mock_dev", host_timestamp_us=1000)
        f2 = Frame(color=np.zeros((1,1,3), dtype=np.uint8), depth=np.zeros((1,1), dtype=np.uint16), timestamp_us=2000, device_id="mock_dev", host_timestamp_us=2000)
        f3 = Frame(color=np.zeros((1,1,3), dtype=np.uint8), depth=np.zeros((1,1), dtype=np.uint16), timestamp_us=3000, device_id="mock_dev", host_timestamp_us=3000)
